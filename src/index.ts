...
function startAcpServer() {
    ...
    acp.agent({name: packageJson.name})
        ...
        .onRequest(acp.methods.agent.session.new, (ctx) => getAgent().newSession(ctx.params, ctx.requestId))
        .onRequest(acp.methods.agent.session.load, (ctx) => getAgent().loadSession(ctx.params, ctx.requestId))
        .onRequest(acp.methods.agent.session.list, (ctx) => getAgent().listSessions(ctx.params, ctx.requestId))
        .onRequest(acp.methods.agent.session.delete, (ctx) => getAgent().deleteSession(ctx.params))
        .onRequest(acp.methods.agent.session.resume, (ctx) => getAgent().resumeSession(ctx.params, ctx.requestId))
